...
public class StudentTerster {

	public static void testTask1() {
		class Blob extends Object3D {
			public Blob(String _name, Object3D.Material _matType) {
	...
	Sphere s = (Sphere)waxDieB.convertToOtherShape();
		...
		System.out.println();
		System.out.println("Converting WaxDie B to a Sphere with radius: "+String.format("%,.2f", 
		s.getRadius())+" and volume: "+String.format("%,.2f", s.getVolume()));

		System.out.println("Converting ButterBall Z to a Cube with edge length: "+String.format("%,.2f", 
		c.getEdge())+" and volume: "+String.format("%,.2f", c.getVolume()));
	}
